/**
 * Preview recorder — records just Scene 3 (pipeline walkthrough)
 * Shows each pipeline agent running, hovering for tooltips,
 * rejection/retry loops, and final completion.
 *
 * Usage:
 *   node preview-scene3.js              → headless
 *   node preview-scene3.js --headed     → watch in real-time
 *   node preview-scene3.js --no-vite    → skip Vite (assume running)
 *
 * Output:
 *   docs/demo/preview-scene3.webm
 */
...
// Scene
import scenePipelineWalkthrough from './scenes/scene-pipeline-walkthrough.js';
...
async function main() {
    console.log('═══════════════════════════════════════════════');
    console.log('  Scene 3 Preview — Pipeline Walkthrough');
    console.log('═══════════════════════════════════════════════\n');
    ...
    // ── Warm up Vite: visit the investigation detail page ──
    ...
    try {
        ...
        await new Promise(r => setTimeout(r, 1500));
        await warmPage.goto(`http://localhost:${VITE_PORT}/investigation/1749820000000`, { waitUntil: 'networkidle', timeout: 20000 });
        await new Promise(r => setTimeout(r, 1500));
    ...
    // Immediate dark frame — video recording starts at newPage(),
    // so give it a clean dark canvas right away (same pattern as Scenes 1 & 2).
    ...
    try {
        ...
        // ── SCENE 3: Pipeline Walkthrough ──
        console.log('  🎬 Pipeline Walkthrough');
        await scenePipelineWalkthrough(page);
    ...
    const dest = join(DEMO_DIR, 'preview-scene3.webm');
    const videoFiles = readdirSync(DEMO_DIR)
        .filter(f => f.endsWith('.webm') && f !== 'preview-scene3.webm')
