/**
 * Preview recorder — records just Scene 4 (Final Report + Notes)
 * Shows the Final Report tab, scrolls through findings,
 * demonstrates Implement Recommendations, then Notes tab.
 *
 * Usage:
 *   node preview-scene4.js              → headless
 *   node preview-scene4.js --headed     → watch in real-time
 *   node preview-scene4.js --no-vite    → skip Vite (assume running)
 *
 * Output:
 *   docs/demo/preview-scene4.webm
 */
...
// Scene
import sceneFinalReport from './scenes/scene-final-report.js';
...
async function main() {
    console.log('═══════════════════════════════════════════════');
    console.log('  Scene 4 Preview — Final Report & Notes');
    console.log('═══════════════════════════════════════════════\n');
    ...
    // so give it a clean dark canvas right away.
    ...
    try {
        ...
        // ── SCENE 4: Final Report & Notes ──
        console.log('  🎬 Final Report & Notes');
        await sceneFinalReport(page);
    ...
    const dest = join(DEMO_DIR, 'preview-scene4.webm');
    const videoFiles = readdirSync(DEMO_DIR)
        .filter(f => f.endsWith('.webm') && f !== 'preview-scene4.webm')
