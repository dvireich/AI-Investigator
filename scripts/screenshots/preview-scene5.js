/**
 * Preview recorder — records just Scene 5 (Retrospect — Knowledge Improvement)
 * Shows the Retrospect tab with completed analysis, then re-runs the analysis
 * demonstrating agent file reads and proposal generation.
 *
 * Usage:
 *   node preview-scene5.js              → headless
 *   node preview-scene5.js --headed     → watch in real-time
 *   node preview-scene5.js --no-vite    → skip Vite (assume running)
 *
 * Output:
 *   docs/demo/preview-scene5.webm
 */
...
// Scene
import sceneRetrospect from './scenes/scene-retrospect.js';
...
async function main() {
    console.log('═══════════════════════════════════════════════');
    console.log('  Scene 5 Preview — Retrospect & Knowledge Improvement');
    console.log('═══════════════════════════════════════════════\n');
    ...
    try {
        ...
        await new Promise(r => setTimeout(r, 1500));
        await warmPage.goto(`http://localhost:${VITE_PORT}/investigation/1749780000000`, { waitUntil: 'networkidle', timeout: 20000 });
        await new Promise(r => setTimeout(r, 1500));
    ...
    try {
        ...
        console.log('  🎬 Retrospect — Knowledge Improvement');
        await sceneRetrospect(page);
    ...
    // Rename output
    const dest = join(DEMO_DIR, 'preview-scene5.webm');
    const videoFiles = readdirSync(DEMO_DIR)
        .filter(f => f.endsWith('.webm') && f !== 'preview-scene5.webm')
